...
public class ToppyBird extends JPanel
    ...
    static final double BOSS_PHASE3_MAX_HP = 5000; // เลือดไคจูยักษ์ 5000 HP[cite: 7]
    ...
    Clip grandMusic;
    Clip paradigmMusic;
    Clip finalMusic; // เสียงเพลง Final สำหรับ Kaiju
    ...
    void loadSounds() {
        ...
        finalMusic    = loadClip(BASE_PATH + "final.wav");
    ...
    // ฟังก์ชันหรี่เสียงเพลงค่อยๆ เบาลงแล้วเปลี่ยนเพลง
    void fadeOutAndSwitchMusic(Clip oldMusic, Clip newMusic) {
        if (!soundOn) {
            stopMusic(oldMusic);
            playMusic(newMusic);
            return;
        ...
        new Thread(() -> {
            try {
                if (oldMusic != null && oldMusic.isRunning()) {
                    for (float vol = 0.4f; vol >= 0.05f; vol -= 0.05f) {
                        setVolume(oldMusic, vol);
                        Thread.sleep(150);
                    }
                    stopMusic(oldMusic);
                }
    ...
    void startBossTransformation(int nextPhase) {
        ...
        if (bossPhase == 2) {
            fadeOutAndSwitchMusic(grandMusic, paradigmMusic);
        } else if (bossPhase == 3) {
            fadeOutAndSwitchMusic(paradigmMusic, finalMusic); // หรี่เพลงเดิมแล้วเปลี่ยนเป็น final.wav
        }
    }
    ...
    void updateBossTransformation() {
        ...
        if (elapsed >= BOSS_TRANSFORM_DURATION) {
            ...
            if (bossPhase == 2) {
                ...
                currentMaxBossHP = BOSS_PHASE3_MAX_HP; // 5000 HP[cite: 7]
